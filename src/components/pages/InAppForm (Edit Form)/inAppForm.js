...
function InAppForm({ Title, onClose, onSubmit }) {
  ...
  const validatePhases = (phaseRows) => {
    const isPhase1Filled =
      phaseRows[0].year1 || phaseRows[0].year2 || phaseRows[0].TR
    const isPhase2Filled =
      phaseRows[1].year1 && phaseRows[1].year2 && phaseRows[1].TR
    const isPhase3Filled =
      phaseRows[2].year1 && phaseRows[2].year2 && phaseRows[2].TR
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${serverurl}/submission/inapp-form`, {
        method: 'POST',
        headers: {
        ...
        <div className="sectionForm-Content h-[60vh] w-[70vh] bg-white">
          {(() => {
            switch (Title) {
              ...
              case 'Solution':
                return (
                  <Solution formData={formData} handleChange={handleChange} />
                )
              case 'Market Sizing':
                return (
